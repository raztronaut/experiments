...
## 2024-05-19 - Add aria-expanded and aria-label to custom table row toggle buttons
**Learning:** Custom expand/collapse buttons in data tables must include `aria-expanded` attributes and descriptive `aria-label`s to communicate their state effectively to assistive technologies.
**Action:** Always pair `aria-expanded` with descriptive `aria-label`s on expand/collapse buttons.
