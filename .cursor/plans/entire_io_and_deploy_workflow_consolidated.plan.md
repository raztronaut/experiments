...
- [Entire.io ordering analysis](d845272b-099d-41d9-8354-718a4b756df1) -- branch split orphan analysis, removed throwaway branch, handoff sequencing
...
- **93 commits** on local `main`, never pushed to `origin/main`
- **Entire CLI**: not installed, no `.entire/` directory
- **lefthook**: switched to `ultracite check` (read-only mode) with safety comment; `stage_fixed: true` is permanently banned (causes stash data loss)
...
### 1e. Add Conventional Commits validation

Add `commit-msg` hook to `lefthook.yml`. The pre-commit block already uses `ultracite check` (read-only mode) -- do NOT change it to `fix` with `stage_fixed: true` (causes stash data loss).

```yaml
# SAFETY: never use `stage_fixed: true` with a formatter/fixer (ultracite fix,
# biome fix, prettier --write). Lefthook stashes unstaged changes before running,
# and if the stash pop fails (formatting touched overlapping lines), unstaged work
# is silently lost as an orphaned stash. Use `check` mode instead — fail fast,
# fix manually.
#
...
git commit -m "chore: integrate entire.io + conventional commits hook"
```
...
```bash
git branch v2/platform          # snapshot HEAD (94+ commits)
git checkout main
...
94 commits covering the complete V2 platform rebuild:
...
| 94-commit PR is large                           | Draft PR allows incremental review. Preview URL provides live testing. CI validates build.                  |
...
- `lefthook.yml` (add commit-msg hook, add Entire ownership comment)
