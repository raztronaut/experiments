...
The full animation runs in five phases -- boot-up, distortion ramp, hold, fade-out, logo reveal -- all in a single timeline. Panels are sorted by Manhattan distance from center for the stagger: `Math.abs(col - 2) + Math.abs(row - 1)`. Center first, corners last. On fade-out, it reverses.
...
The `EffectComposer` gets recreated on every resize because it's in a `useMemo` that depends on `size`. Should create it once and call `setSize` instead.
