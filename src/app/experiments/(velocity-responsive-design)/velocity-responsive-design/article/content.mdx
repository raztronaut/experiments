...
The fix comes from electrical engineering. A [Schmitt trigger](https://en.wikipedia.org/wiki/Schmitt_trigger) uses **hysteresis** — different thresholds for entering and exiting a state. The gap between them creates a dead zone where the current state holds.
...
The yellow regions are skim mode. The gap between the red enter line and the blue exit line is the dead zone — velocity can bounce around in there without triggering a transition. Drag the thresholds to see how the dead zone width affects stability.
...
The velocity engine taps into [Lenis](https://lenis.darkroom.engineering/) smooth scroll. Lenis provides a native `velocity` property on every scroll event — no need to compute deltas manually.
...
Scroll slowly to read, quickly to skim. The FlightControl dashboard at the bottom shows your velocity in real-time. Click the gear icon to toggle manual override — drag the slider to sweep through velocities and watch the content adapt.
...
The FlightControl dashboard started as a debug tool for calibrating thresholds. It turned out to be the best way to understand what the experiment does — you can sweep through the velocity range and watch every element respond. Sometimes the debug tool *is* the feature.
