...
const CAT_BODY = [
  ["     /\\_ /\\", " ___ / o o \\", "/ ___   =-= /", "\\____)-m-m)"],
  ["     /\\_/\\", " ___/ o o \\", "/___   =-= /", "\\____)mm__)"],
  ["     /\\_/\\", " ___/ · · \\", "/___   =-= /", "\\___)-m__m)"],
  ["     /\\_/\\", " ___/ o o \\", "/___   =-= /", "\\____)-mm-)"],
];
...
export const useConsoleCat = (enabled = true) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < fullTerrainWidth; i++) {
      baseTerrain +=
        SNOW_TERRAIN_CHARS[
          Math.floor(Math.random() * SNOW_TERRAIN_CHARS.length)
        ];
    }
    ...
    const render = () => {
      ...
      if (state.speech.visible) {
        ...
        if (Math.random() < 0.05) {
          ...
          state.speech.text =
            CHRISTMAS_PHRASES[
              Math.floor(Math.random() * CHRISTMAS_PHRASES.length)
            ];
      ...
      body.forEach((line, index) => {
        ...
        const hatColor =
          "color: #ff5555; font-weight: bold; font-family: monospace;"; // Red
        const bodyColor =
          "color: #ff79c6; font-weight: bold; font-family: monospace;"; // Pink
