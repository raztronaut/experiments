...
const ReplayGrid = forwardRef<ReplayGridHandle, ReplayGridProps>(
  function ReplayGrid({ bgColor = "#f7f7f9", isDark = false }, ref) {
    ...
    for (let row = 0; row < ROWS; row++) {
      for (let col = 0; col < COLS; col++) {
        ...
        const imageSrc = isLogo
          ? "/experiments/basketball-replay-center/pickandroll-logo.svg"
          : undefined;
        // Pad the clip ID with zeros e.g. "01", "02"
        const clipNum = clipIdx.toString().padStart(2, "0");
        const videoSrc = isLogo
          ? undefined
          : `/experiments/basketball-replay-center/clips/clip_${clipNum}.mp4`;
        if (!isLogo) {
          clipIdx++;
        }
          ...
          <group
            ...
            ref={(el: THREE.Group | null) => {
              if (el) {
                ...
                if (mesh) {
                  panelRefs.current.set(key, mesh);
                }
              }
            }}
          >
