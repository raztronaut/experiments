...
export default class VisualiserLogic {
    ...
    async fetchCovers() {
        ...
        const urls: string[] = new Array(20)
            .fill(0)
            .map((_, i) => `/experiments/rabbithole-chat-preloader/frames/${i + 1}.jpg`)
    ...
    private _onPointerDown = (e: PointerEvent) => {
        ...
        if (this.dragElement) this.dragElement.setPointerCapture(e.pointerId)
    }
    ...
    private _onPointerUp = (e: PointerEvent) => {
        ...
        try {
            if (this.dragElement) this.dragElement.releasePointerCapture(e.pointerId)
        } catch { }
    }
    ...
    handleClick(clientX: number, clientY: number) {
        ...
        for (let i = 0; i < this.meshCount; i++) {
            ...
            const zDisp = shaderMod(uScrollY + minZoffset, maxZoffset + minZoffset) - minZoffset
            ...
            if (Math.abs(denom) < 1e-6) continue // Parallel to plane

            const t = (currentZ - rayOrigin.z) / denom
            if (t < 0) continue // Behind ray start

            // Intersection point
            const intersectPoint = new THREE.Vector3().copy(rayOrigin).addScaledVector(rayDir, t)
            ...
            const xDisp = shaderMod(minXoffset - uDrag.x + uTime * speed, maxXoffset + minXoffset) - minXoffset
            const yDisp = shaderMod(minYoffset - uDrag.y, maxYoffset + minYoffset) - minYoffset
        ...
        if (hitIndex !== -1) {
            window.open("https://www.rabbithole.chat/rabbithole/how-did-humans-first-7kjxug9e", "_blank")
        }
    }
    ...
    render(delta: number) {
        ...
        this.drag.xCurrent +=
            (this.drag.xTarget - this.drag.xCurrent) * this.dragDamping /* * normalizedDelta? Reference didn't use it for lerp but maybe implicitly expected it? */
        // Reference code: 
