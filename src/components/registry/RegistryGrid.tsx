...
interface RegistrySlimItem {
  ...
  type?: string;
  video?: string | null;
  _searchString?: string; // Pre-computed lowercase string for faster filtering
}
  ...
  const items = useMemo(
    () =>
      rawItems
        ...
        .map((item) => ({
          ...item,
          // Pre-compute search string to avoid repeated .toLowerCase() allocations during filtering
          _searchString: [
            item.title,
            item.description,
            ...(item.tags ?? []),
