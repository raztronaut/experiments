...
export const getAdjacentArticles = cache(async (experimentSlug: string) => {
  const articles = await getArticles();
  const indexMap = await getArticleIndexMap();

  // ⚡ Bolt Optimization: Use precomputed Map for O(1) index lookup
  // instead of O(N) Array.findIndex, improving speed across multiple calls
  const idx = indexMap.get(experimentSlug) ?? -1;
