...
function calculateReadingMinutes(text: string): number {
  // Optimization: Instead of using `reading-time-estimator` which uses an HTML parser internally,
  // we use a simple regex to count words. This is significantly faster and prevents GC spikes
  // during build and feed generation when processing multiple MDX articles.
...
export const getArticleContent = cache(
  async (slug: string): Promise<ArticleContent | null> => {
    ...
    try {
      ...
      const { data, content } = matter(raw);
      const estimateMinutes = calculateReadingMinutes(content);
      return { frontmatter: data, content, readingMinutes: estimateMinutes };
    } catch {
      return null;
    }
  }
