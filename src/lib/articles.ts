...
// ⚡ Bolt: Cache deduplicates by arguments. By precomputing a Map of indices in a
// parameterless cache block, we avoid the O(N^2) bottleneck caused by repeatedly
// executing O(N) array searches for different keys within the cache boundary.
const getArticleIndices = cache(async () => {
...
export const getAdjacentArticles = async (experimentSlug: string) => {
  const { articles, map } = await getArticleIndices();
  // ⚡ Bolt: Replace O(N) findIndex loop with O(1) Map lookup
