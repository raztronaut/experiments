...
async function optimizeVideos() {
    ...
    for (const videoPath of videos) {
        ...
        if (sizeMB < 2) {
            console.log(`⏩ Skipping small video: ${path.basename(videoPath)} (${sizeMB.toFixed(2)} MB)`);
            continue;
        }
        ...
        try {
            ...
            console.log(`✅ Optimized! ${sizeMB.toFixed(2)}MB -> ${newSizeMB.toFixed(2)}MB (${savings}% saved)`);

            // Overwrite original
            fs.renameSync(tempPath, videoPath);

        } catch (error) {
            console.error(`❌ Failed to optimize ${path.basename(videoPath)}:`, error.message);
